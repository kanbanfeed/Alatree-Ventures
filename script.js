// CORRECTED JAVASCRIPT with proper null checks
...
const onScroll = () => {
  ...
  if (hdr) {
    if (window.scrollY > 30) {
      ...
      if (topbar) topbar.style.padding = '12px 0';
    } else {
      hdr.classList.remove('scrolled');
      const topbar = document.querySelector('.topbar');
      if (topbar) topbar.style.padding = '20px 0';
    }
  }
};

// Only add scroll listener if we're on a page with the header
if (hdr || progress) {
...
// Initialize interactive map - Only if element exists
